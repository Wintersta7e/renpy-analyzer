...
RE_PRECEDENCE_BUG = re.compile(r"\b(\w+)\s+(or|and)\s+(\w+)\s*==\s*(True|False)\b")

RE_EXPLICIT_BOOL = re.compile(r"\b(\w+)\s*==\s*(True|False)\b")
        ...
        for m in RE_EXPLICIT_BOOL.finditer(expr):
            ...
            findings.append(
                Finding(
                    ...
                    file=cond.file,
                    line=cond.line,
                    suggestion=(f"Use `{'not ' if bool_val == 'False' else ''}{var_name}` instead."),
                )
            )
